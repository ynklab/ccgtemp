    ...
    def get_semantic_representation(self, ccg_tree, tokens):
        ...
        if len(ccg_tree) == 0:
            ...
            for path in var_paths:
                ...
                if str(semantics)[1] in ['L','R'] and str(semantics)[2].isdigit():
                    if len(child_node) == 0:
                        child_base = lexpr(child_node.get('base'))
                        semantics = semantics(child_base).simplify()
                    elif len(child_node) == 1:
                        child_base = lexpr(child_node.get('child0_base'))
                        semantics = semantics(child_base).simplify()
                    else:
                        child0_base = lexpr(child_node.get('child0_base'))
                        child1_base = lexpr(child_node.get('child1_base'))
                        semantics = semantics(child0_base).simplify()
                        semantics = semantics(child1_base).simplify()
                else:
